...
private:
    ...
    [[gnu::always_inline]] inline HashProbeType GetProbe(uint64_t hash)
    {
    ...
    {        
        ...
        for (uint64_t i = 0; i < old_table_size; ++i)
        {
            ...
            uint32_t mask = ~(uint32_t)_mm256_movemask_epi8(cmp_zero);
            ...
            if constexpr (Flags & STORE_HASH)
            {
                while (mask)
                {
                    ...
                    {   
                        // copy into same cell, becouse it is 100% unused
                        SetCell(old_table_size + i, t, data[i].keys[t], data[i].values[t], hash, data[i].probes[t]);
                        data[i].probes[t] = 0;
                    }
    ...
    Value *ptr_insert(const Key &key, const Value &value)
    {
        ...
        while (true)
        {
            ...
            if constexpr ((Flags & STORE_HASH) != 0)
            {
                ...
                {
                    SetCell(i, first_empty_t, key, value, hash, probe);
                    count++;
