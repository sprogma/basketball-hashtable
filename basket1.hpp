...
public:
    basketball()
    {
        probes = (ProbeBasket *)calloc(1, sizeof(*probes) * 16);
        keys = (Key *)calloc(1, sizeof(*keys) * 16 * ProbeBasketSize);
        values = (Value *)calloc(1, sizeof(*values) * 16 * ProbeBasketSize);
        if constexpr (Flags & STORE_HASH)
        {
            this->hashes = (uint64_t *)calloc(1, sizeof(*this->hashes) * 16 * ProbeBasketSize);
        }
        count = 0;
        table_size = 16;
    }
...
private:
    ...
    {        
        ...
        table_size *= 2;
        probes = (ProbeBasket *)realloc(probes, sizeof(*probes) * table_size);
        keys = (Key *)realloc(keys, sizeof(*keys) * linear_size());
        values = (Value *)realloc(values, sizeof(*values) * linear_size());
        if constexpr (Flags & STORE_HASH)
        {
            this->hashes = (uint64_t *)realloc(this->hashes, sizeof(*this->hashes) * linear_size());
            memset(this->hashes + old_table_size * ProbeBasketSize, 0, sizeof(*this->hashes) * old_table_size * ProbeBasketSize);
        }
        memset(probes + old_table_size, 0, sizeof(*probes) * old_table_size);
